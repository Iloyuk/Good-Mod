...
class UltEnchantsGui : GuiScreen() {

    // should contain itemNames, and the corresponding buttonId should be items.indexOf(itemName)
    private val items: List<Pair<String, String>> = AuctionParser.items["ults"]?.get("bazaar")?.toList() ?: emptyList()
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        drawCenteredString(fontRendererObj, "Ultimate Enchantments", width / 2, 20, 0x00FFFF)
    ...
    companion object {
        fun open() {
            GoodMod.display = UltEnchantsGui()
        }
    }
}