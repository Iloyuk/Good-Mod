...
class Floor3Gui : GuiScreen() {

    // should contain itemNames, and the corresponding buttonId should be items.indexOf(itemName)
    private val items: List<Pair<String, String>> = AuctionParser.items["floor_3"]?.get("bazaar")?.toList() ?: emptyList()
    ...
    override fun initGui() {
        ...
        renderRows(items, width, height, listOf(0x5555FF, 0xAA00AA)).forEach { buttonList.add(it) }
    }
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        drawCenteredString(fontRendererObj, "Floor 3", width / 2, 20, 0x00FFFF)
