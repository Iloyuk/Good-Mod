...
class Floor7Gui : GuiScreen() {

    // should contain itemNames, and the corresponding buttonId should be items.indexOf(itemName)
    private val items: List<Pair<String, String>> = AuctionParser.items["floor_7"]?.get("bazaar")?.toList() ?: emptyList()
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        drawCenteredString(fontRendererObj, "§lFloor 7", width / 2, 20, 0xFF55555)
    ...
    // allows gui to be opened with Floor7Gui.open()
    companion object {
        fun open() {
            GoodMod.display = Floor7Gui()
        }
    }
}