...
object CroesusChestParser {
    ...
    fun parseCroesusLoot(chest: ContainerChest) {
        ...
        for (index in 9..17) {
            ...
            if (costIndex != null) {
                ...
                runLoot.add(CroesusChest(displayName, lootSubList, false, chestCost))
            } else {
                runLoot.add(CroesusChest(displayName, lootSubList, true, 0.00))
    ...
    private fun findCost(tagList: NBTTagList, index: Int): Double {
        ...
        val cost = when {
            firstLine == "§aFREE" -> 0
            firstLine == "§9Dungeon Chest Key" -> dungeonChestKeyPrice
